    ...
    <main className="home">
      <div className="container mx-auto px-20 h-screen flex flex-wrap justify-center items-center">
        <div className="w-full md:w-3/12" >
          ...
          <img alt="fullstack.chat logo" src="/assets/images/logo-2.png" className="logo mx-auto" />
      ...
      <div className="container mx-auto px-4 lg:px-20 justify-center items-center pt-12 flex text-white">
        <div className="flex flex-col w-full md:w-1/2">
          ...
          <div className="rules">
            <div className="rules-intro">Before joining, read and agree to the rules below. Help keep our server friendly and welcoming!</div>
            <div className="rules-container">
              <h3>NSFW Content</h3>
              <p>No NSFW content is permitted here. Posting NSFW content will result in an immediate ban without warning.</p>

              <h3>Accept Others</h3>
              <p>Be kind and respectful of all members. We are tolerant of all races, genders, LGBTQ, religions, and creeds.</p>

              <h3>Addressing Conflicts</h3>
              <p>If you take issue with another member, discuss it with them PRIVATELY to come to a resolution. If you have done so and the issue is not yet resolved, the admins & mods are always open to assist in any way we can. It’s also worth noting that mods are not monitoring all channels 24/7, so members are expected to report issues if a mod is not available.  </p>

              <h3>Logging</h3>
              <p>Actions that modify or delete content from the Discord are logged for reference if needed. This includes the content of messages PRIOR to modification, as well as the modified version of the message. </p>

              <h3>Constructive Criticism</h3>
              <p>Mods and the direction of the community are open to constructive criticism. If you feel the group is heading in the wrong direction or can be improved in some way, our minds will always be open to considerations. Blatant criticism and insulting mods, however, will not be tolerated and will result in a ban from the server. </p>

              <h3>No Patronizing</h3>
              <p>Feel free to celebrate wins and progress in your coding journey, but dont talk down to other members or make them feel beneath you or anyone else. Direct comparison of your own progress to others in a negative context will not be tolerated.</p>

              <h3>Trolling</h3>
              <p>Trolling can be good natured fun, but can also sincerely offend people if they take it the wrong way. Keep trolling to a minimum and if you are offended by someone’s comments, discuss it with them to cultivate a positive outcome.</p>
            </div>

            <div className="rules-footer flex flex-col items-center">
              <div className="form-check agree-to-rules-check" >
                <input type="checkbox" className="form-check-input" checked={isUserAgreedToRules} onChange={() => setIsUserAgreedToRules(!isUserAgreedToRules)} />
                <label className="form-check-label ml-1">I read and agree to the rules above</label>
              </div>
              <button type="button"
                className={`agree-to-rules-button rounded-md py-1 px-3 ${isUserAgreedToRules ? "" : "agree-to-rules-button-disabled"}`}
                disabled={!isUserAgreedToRules}
                onClick={() => goToDiscord()}>Join fullstack.chat!</button>
            </div>
          </div>
        </div>
      </div>
